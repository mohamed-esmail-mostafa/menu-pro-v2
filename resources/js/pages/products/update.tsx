...
import { ArrowLeft, ArrowRight, Edit, Eye, Loader2, Package, PlusCircle, Save, Sliders, Sparkles, Star, Tag, Trash2, Utensils } from 'lucide-react'
...
export default function UpdateProductPage({ product, store, categories, attributes }: UpdateProductPageProps) {
    const { t, isAr } = useImport()
    const [imageFile, setImageFile] = useState<File | null>(null)
    const [imagePreviewUrl, setImagePreviewUrl] = useState<string | null>(product?.image || null)

    const initialAttributeRows = product?.attributes?.flatMap((attribute: any) =>
        attribute?.values?.map((value: any) => ({
            id: value.id,
            attribute_id: String(attribute.id || value.attribute_id || ''),
    ...
    const formik = useFormik({
        initialValues: {
            ...
            store_category_id: product?.store_category_id ? product.store_category_id.toString() : '',
            title: product?.title || '',
            description: product?.description || '',
            price: product?.price ? product.price.toString() : '',
            sale_price: product?.sale_price ? product.sale_price.toString() : '',
            is_simple: product?.is_simple ?? true,
        ...
        onSubmit: (values) => {
            ...
            router.post(`/store/product/update/${product.id}`, formData, {
                onSuccess: () => {
                    toast.success(t('store_dashboard.products.updated-success'))
                },
                onError: (errors: any) => {
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b pb-4">
                    <div className="space-y-1">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs text-muted-foreground">
                                ...
                                {' / '}
                                <span className="text-foreground font-medium">{t('store_dashboard.products.edit-title')}</span>
                            </span>
                        </div>
                        <h1 className="text-2xl font-bold tracking-tight flex items-center gap-2 mt-1">
                            <Edit className="w-6 h-6 text-primary" />
                            {t('store_dashboard.products.edit-title')}
                        </h1>
                        <p className="text-xs text-muted-foreground">
                            {t('store_dashboard.products.edit-subtitle')}
                        </p>
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        >
                            ...
                            {t('store_dashboard.products.save-changes')}
                        </Button>
                    </div>
                </div>
                            ...
                            <Card className="rounded-2xl border shadow-sm">
                                ...
                                <CardContent className="p-6">
                                    <ImagePicker
                                        ...
                                        initialPreview={product?.image || undefined}
                                    />
                                </CardContent>
                            </Card>
